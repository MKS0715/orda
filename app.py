...
def generate_item_feedback(records_df, item_name):
    ...
    if "플랭크" in item_name:
        if diff > 10:
            ...
            return f"📈 {item_short}이 조금씩 늘고 있어요. 꾸준히 하면 더 좋아질 거예요! 👍"
        elif diff == 0:
            return f"📊 {item_short} 기록이 유지되고 있어요. 조금만 더 도전해볼까요? 😊"
        else:
            return f"💡 {item_short}이 살짝 줄었어요. 매일 30초씩 연습하면 금방 회복돼요! 💪"
    elif "왕복달리기" in item_name:
        if diff > 5:
            return f"🔥 {item_short} {diff:.0f}회 증가! 심폐지구력이 많이 좋아졌어요! 🏃"
        elif diff > 0:
            return f"📈 {item_short}이 조금씩 늘고 있어요. 꾸준한 달리기가 효과를 보고 있어요! 👍"
        elif diff == 0:
            return f"📊 {item_short} 기록이 유지 중이에요. 매일 조금씩 더 뛰어볼까요? 😊"
        else:
            return f"💡 {item_short}이 약간 줄었지만 괜찮아요! 가볍게 달리기 연습해보세요! 🏃"
    elif "사이드스텝" in item_name:
        if diff > 3:
            return f"🔥 {item_short} {diff:.0f}회 증가! 순발력이 눈에 띄게 좋아졌어요! ⚡"
        elif diff > 0:
            return f"📈 {item_short}이 향상되고 있어요! 민첩성이 좋아지고 있는 증거예요! 👍"
        elif diff == 0:
            return f"📊 {item_short} 기록이 안정적이에요. 줄넘기로 스텝 연습해볼까요? 😊"
        else:
            return f"💡 {item_short}이 조금 줄었어요. 좌우 스텝 연습을 하면 금방 늘 거예요! ⚡"
    elif "굽히기" in item_name:
        if diff > 3:
            return f"🔥 {item_short} {diff:.1f}cm 향상! 유연성이 정말 좋아졌어요! 🧘"
        elif diff > 0:
            return f"📈 {item_short}이 조금씩 늘고 있어요! 스트레칭 효과가 나타나고 있어요! 👍"
        elif diff == 0:
            return f"📊 {item_short} 기록이 유지 중이에요. 매일 스트레칭 습관을 들여볼까요? 😊"
        else:
            return f"💡 {item_short}이 살짝 줄었어요. 매일 10초씩 스트레칭하면 금방 좋아져요! 🧘"
    return None
...
def show_record_input(client, info, records):
    ...
    with col_round:
        round_num = st.selectbox("측정 회차", [1, 2, 3, 4, 5, 6, 7, 8])
    with col_date:
            ...
            with st.form("admin_record_form"):
                ...
                with col1:
                    grade = st.selectbox("학년", sorted(students["학년"].astype(str).unique()),
                                        format_func=lambda x: f"{x}학년", key="ar_grade")
                ...
                with col2:
                    round_num = st.selectbox("측정 회차", [1, 2, 3, 4, 5, 6, 7, 8], key="ar_round")
